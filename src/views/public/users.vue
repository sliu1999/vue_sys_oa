<template>
  <div>
    ...
    <div style="float: left;">
      <div v-if="!flagMultiple" style="margin-left: 5px;line-height: 32px;"><span>{{displayName}}</span><i v-if="displayName" class="el-icon-error" @click="doRemove()" style="margin-left:10px;color: #F56C6C"></i></div>
      <div v-if="flagMultiple">
        <el-tag
          v-for="(selectedUser,index) in multipleSelection"
          :key="selectedUser.trueName"
          closable
          @close="handleClose(selectedUser,index)"
          type=""
          style="margin-left: 2px;line-height:32px;">
...
<script>
  ...
  export default {
    ...
    created:function(){
      ...
      this.initGroup();
      //this.initUserGroup();
      if(this.val){                                                      //若加载组件时有传入的值，则根据配置相应显示
        let _this=this;
        //根据用户IDS获取用户信息
    ...
    methods:{
      init(){
        ...
        queryUserDetailPage(obj).then(
          function (res) {
            ...
            if(_this.val){                                   //打开模态框如果有值，根据值回显数据
      ...
      handleClose(selectedUser,index){
        ...
        // this.$refs.multipleTable.toggleRowSelection(selectedUser);
        this.setMultiOutput();
      },
      doRemove(){
        this.vals = '';
