  ...
  <div>
    <el-tabs type="border-card">
      <el-tab-pane label="明细列表">
        <div>
          {{names}}
          <el-button type="text" size="mini" @click="addDetail()" v-if="detailDatas.length!=0">添加</el-button>
            ...
            <table class="detail-table" style="width:100%;">
              ...
              <tr v-for="(detailData,index) in detailDatas">
                ...
                <td v-for="detail in fieldList" v-if="!detail.fieldHide">
                  <span style="color:red;position:absolute;top:0;right:0;font-size: 25px;" v-if="detail.required&&detailData.hasOwnProperty(detail.id)&&detail.id">*</span>
                  <component v-if="detailData.hasOwnProperty(detail.id)||!detail.id" :is="detail.html" :val.sync="detailData[detail.id]"
        ...
        <div style="width:100%;overflow-x: auto">
          <table class="detail-table" style="width:100%;">
            ...
            <tr v-for="(detailData,index) in sumDatas">
              ...
              <td v-for="detail in fieldList" v-if="!detail.fieldHide">
                <span style="color:red;position:absolute;top:0;right:0;font-size: 25px;" v-if="detail.required&&detailData.hasOwnProperty(detail.id)&&detail.id">*</span>
                <component v-if="detailData.hasOwnProperty(detail.id)||!detail.id" :is="detail.sumHtml" :val.sync="detailData[detail.id]"
                           :name.sync="detail.name" :config.sync="detail.config" :id.sync="detail.id" :index="index" :parent.sync="detailDatas"
                           :grandparent.sync="parents" :tableKey.sync="tableKey"></component>
                <!-- is绑定组件名称  val绑定表单字段的值   name绑定表单字段的名称   config绑定各组件的配置信息   id绑定表单字段的id   index绑定明细表的数组下标   parent绑定整个明细表数组的数据    grandparent绑定整个表单数据（如sql表单域里需要主表里的字段数据） tableKey绑定当前表单表名 -->
                <p v-if="!detailData.hasOwnProperty(detail.id)&&detail.id" style="color: #ccc;">用户无权限</p>
      ...
      init() {
        ...
        this.fields.forEach(field => {
          ...
          if (field.default) {
            ...
            this.originDetail[field.id] = null;
          }
        })
